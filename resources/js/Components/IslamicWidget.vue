...
<template>
    <div class="bg-gradient-to-br from-amber-400 to-amber-500 rounded-2xl p-6 shadow-md text-white relative overflow-hidden">
        <!-- Background Decoration (Islamic Ornament) -->
        <div class="absolute -right-8 -top-8 opacity-20">
            <svg class="w-48 h-48 text-amber-300" viewBox="0 0 100 100" fill="none" stroke="currentColor" stroke-width="2">
                <!-- Square 1 -->
        ...
        <div class="relative z-10">
            <!-- Header & Date -->
            <div class="mb-6">
                <h3 class="text-sm font-bold text-amber-100 uppercase tracking-widest mb-1">{{ locationName }}</h3>
                <div v-if="isLoading" class="h-8 bg-amber-500/50 rounded animate-pulse w-3/4"></div>
                <div v-else-if="error" class="text-sm text-rose-200">{{ error }}</div>
                <div v-else>
                    <div class="text-lg font-bold text-white mb-1">{{ masehiDate }}</div>
                    <div class="text-xl font-medium tracking-tight text-amber-50">{{ hijriDate }}</div>
                </div>
                <p class="text-xs text-amber-200 mt-3 font-medium">Jadwal Sholat Harian</p>
            </div>
            ...
            <div class="grid grid-cols-2 gap-3" v-if="!isLoading && prayerTimes">
                <div class="bg-white/10 backdrop-blur-sm border border-white/20 rounded-xl p-3 flex justify-between items-center hover:bg-white/20 transition-colors">
                    <span class="text-xs font-bold text-amber-50 uppercase tracking-wider">Subuh</span>
                    <span class="text-sm font-black text-white">{{ prayerTimes.Subuh }}</span>
                </div>
                <div class="bg-white/10 backdrop-blur-sm border border-white/20 rounded-xl p-3 flex justify-between items-center hover:bg-white/20 transition-colors">
                    <span class="text-xs font-bold text-amber-50 uppercase tracking-wider">Terbit</span>
                    <span class="text-sm font-black text-white">{{ prayerTimes.Terbit }}</span>
                </div>
                <div class="bg-white/10 backdrop-blur-sm border border-white/20 rounded-xl p-3 flex justify-between items-center hover:bg-white/20 transition-colors">
                    <span class="text-xs font-bold text-amber-50 uppercase tracking-wider">Dzuhur</span>
                    <span class="text-sm font-black text-white">{{ prayerTimes.Dzuhur }}</span>
                </div>
                <div class="bg-white/10 backdrop-blur-sm border border-white/20 rounded-xl p-3 flex justify-between items-center hover:bg-white/20 transition-colors">
                    <span class="text-xs font-bold text-amber-50 uppercase tracking-wider">Ashar</span>
                    <span class="text-sm font-black text-white">{{ prayerTimes.Ashar }}</span>
                </div>
                <div class="bg-white/10 backdrop-blur-sm border border-white/20 rounded-xl p-3 flex justify-between items-center hover:bg-white/20 transition-colors">
                    <span class="text-xs font-bold text-amber-50 uppercase tracking-wider">Maghrib</span>
                    <span class="text-sm font-black text-white">{{ prayerTimes.Maghrib }}</span>
                </div>
                <div class="bg-white/10 backdrop-blur-sm border border-white/20 rounded-xl p-3 flex justify-between items-center hover:bg-white/20 transition-colors">
                    <span class="text-xs font-bold text-amber-50 uppercase tracking-wider">Isya</span>
            ...
            <div class="mt-6 pt-6 border-t border-amber-500/50">
                ...
                <p class="text-xs font-bold text-amber-200 mt-2">— {{ hadith.source }}</p>
            </div>
        </div>
    </div>
</template>
