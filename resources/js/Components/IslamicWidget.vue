...
<template>
    <div class="bg-gradient-to-br from-emerald-600 to-emerald-800 rounded-2xl p-6 shadow-md text-white relative overflow-hidden" :class="{'md:p-8': horizontal}">
        ...
        <div class="relative z-10 h-full" :class="{'md:flex md:items-center md:gap-8': horizontal, 'flex flex-col': !horizontal}">
            <!-- Header & Date -->
            ...
            <!-- Prayer Times Grid -->
            <div :class="['grid gap-2 md:gap-3', horizontal ? 'grid-cols-3 lg:grid-cols-6 flex-1' : 'grid-cols-2']" v-if="!isLoading && prayerTimes">
            ...
            <div :class="['grid gap-3', horizontal ? 'grid-cols-3 lg:grid-cols-6 flex-1' : 'grid-cols-2']" v-if="isLoading">
