...
// --- Helper untuk Ikon Berdasarkan Template Habit ---
const getHabitIcon = (template) => {
    switch(template) {
        case 'sholat_wajib':
        case 'sholat_rawatib':
            // Ikon Sajadah / Masjid (simbol sholat)
            return '<path stroke-linecap="round" stroke-linejoin="round" d="M12 21v-8.25M15.75 21v-8.25M8.25 21v-8.25M3 9l9-6 9 6m-1.5 12V10.332A48.36 48.36 0 0012 9.75c-2.551 0-5.056.2-7.5.582V21M3 21h18M12 6.75h.008v.008H12V6.75z" />';
        case 'quran':
        case 'hadist':
            // Ikon Buku Terbuka (Al-Quran/Hadist)
        ...
        case 'buku':
            // Ikon Bookmark
            return '<path stroke-linecap="round" stroke-linejoin="round" d="M17.593 3.322c1.1.128 1.907 1.077 1.907 2.185V21L12 17.25 4.5 21V5.507c0-1.108.806-2.057 1.907-2.185a48.507 48.507 0 0111.186 0z" />';
        default:
            // Ikon Centang (Default/Boolean)
            ...
            <!-- <div class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm p-6 sm:p-8 relative overflow-hidden">
                ...
                <div class="relative z-10">
                    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                        ...
                        <!-- Header Section -->
            <!-- Ubah padding dari p-6 menjadi p-3 untuk mobile, dan p-6/p-8 untuk tablet/desktop -->
            <div class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm p-3 md:p-6 sm:p-8 relative overflow-hidden mb-2">
                <div class="absolute -right-20 -top-20 w-64 h-64 bg-emerald-50 rounded-full blur-3xl pointer-events-none" aria-hidden="true" />
                
                <div class="relative z-10">
                    <div class="flex flex-col md:flex-row md:items-center justify-between gap-0 md:gap-4">
                        
                        <!-- Judul: DIBUAT HILANG DI MOBILE (hidden) TAPI MUNCUL DI DESKTOP (md:block) -->
                        <div class="hidden md:block">
                            <h1 class="text-2xl font-bold text-slate-800 dark:text-slate-200">Laporan Ibadah Harian</h1>
                            <p class="text-slate-500 dark:text-slate-400 mt-1">Lengkapi tabel capaian Anda di bawah ini.</p>
                            
                            <!-- Indikator Haid untuk Desktop -->
                            <span v-if="isSedangHaid" class="inline-flex items-center gap-1.5 mt-3 px-3 py-1 bg-pink-50 text-pink-700 text-xs font-bold rounded-full border border-pink-100">
                                <span class="w-1.5 h-1.5 rounded-full bg-pink-500 animate-pulse" />
                                Mode Haid Aktif
                            </span>
                        ...
                        <!-- Indikator Haid untuk Mobile (Muncul di atas tanggal jika sedang haid) -->
                        <div v-if="isSedangHaid" class="md:hidden mb-2">
                             <span class="inline-flex items-center gap-1.5 px-3 py-1 bg-pink-50 text-pink-700 text-[10px] font-bold rounded-full border border-pink-100">
                                <span class="w-1.5 h-1.5 rounded-full bg-pink-500 animate-pulse" />
                                Mode Haid Aktif
                            </span>
                        </div>
                        
                        <!-- Date Picker: Menjadi fokus utama di Mobile -->
                        <div class="w-full md:w-auto bg-slate-50 dark:bg-slate-800/50 p-3 md:p-4 rounded-xl border border-slate-200 dark:border-slate-700">
                            <label class="block text-[10px] md:text-xs font-bold text-slate-500 dark:text-slate-400 mb-1.5 uppercase tracking-wider">Tanggal Laporan</label>
                            <input 
                                type="date" 
                                v-model="selectedDate"
                                :min="minDateStr"
                                :max="maxDateStr"
                                class="w-full sm:w-auto px-3 py-1.5 md:px-4 md:py-2 border-slate-300 dark:border-slate-600 rounded-lg text-sm font-bold text-slate-700 dark:text-slate-300 focus:ring-emerald-500 focus:border-emerald-500 dark:bg-slate-700"
                            />
                            <!-- Keterangan batas tanggal disembunyikan di mobile agar lebih ringkas -->
                            <p class="hidden md:block text-[10px] text-slate-400 mt-1 max-w-[200px]">Pilih tanggal laporan (Batas: maks tgl 4 untuk bulan lalu).</p>
                        </div>
                    </div>
                </div>
            </div>
